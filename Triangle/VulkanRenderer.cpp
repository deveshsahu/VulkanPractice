...
void VulkanRenderer::postLoop()
{
	vkDeviceWaitIdle(myLogicalDevice);
}

void VulkanRenderer::resizeCallback(int width, int height)
{
	recreateSwapchain();
}
...
QueueFamilyIndex VulkanRenderer::findQueueFamilies(VkPhysicalDevice device)
{
...
VkFormat VulkanRenderer::findSupportedFormat(const std::vector<VkFormat>& formatPriority, VkImageTiling tiling, VkFormatFeatureFlags features)
{
	for (auto& format : formatPriority)
	{
...
void VulkanRenderer::myInitVulkan()
{
	myCreateInstance();
	setupDebugCallback();
	myCreateSurface();
...
void VulkanRenderer::myPickPhysicalDevice()
{
...
void VulkanRenderer::myCreateImageViews()
{
	mySwapChainImageViews.resize(mySwapChainImages.size());
	for (size_t i = 0; i < mySwapChainImages.size(); ++i)
		mySwapChainImageViews[i] = myCreateImageView(mySwapChainImages[i], mySwapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT);
}

void VulkanRenderer::myCreateRenderPass()
{
...
void VulkanRenderer::myCreateDescriptorSetLayout()
{
...
void VulkanRenderer::myCreateGraphicsPipeline()
{
...
void VulkanRenderer::myCreateFramebuffers()
{
...
void VulkanRenderer::myCreateTextureImageView()
{
	myTextureImageView = myCreateImageView(myTextureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT);
}

void VulkanRenderer::myCreateTextureSampler()
{
...
void VulkanRenderer::myCreateCommandPool()
{
...
void VulkanRenderer::myCreateDepthTestResources()
{
	auto format = findDepthFormat();
	myCreateImage(mySwapChainExtent.width, mySwapChainExtent.height, format, VK_IMAGE_TILING_OPTIMAL,
		VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, myDepthImage, myDepthImageMemory);
	myDepthImageView = myCreateImageView(myDepthImage, format, VK_IMAGE_ASPECT_DEPTH_BIT);

	transitionImageLayout(myDepthImage, format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
}

void VulkanRenderer::myCreateTextureImage()
{
...
void VulkanRenderer::myCreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
...
void VulkanRenderer::myCreateImage(uint32_t width, uint32_t height, VkFormat format,
...
void VulkanRenderer::myBeginSingleTimeCommands(VkCommandBuffer& commandBuffer)
{
...
void VulkanRenderer::myEndSingleTimeCommands(VkCommandBuffer& commandBuffer)
{
...
VkImageView VulkanRenderer::myCreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags)
{
...
void VulkanRenderer::myCreateVertexBuffer()
{
...
void VulkanRenderer::myCreateIndexBuffer()
{
...
void VulkanRenderer::myCreateUniformBuffer()
{
...
void VulkanRenderer::myCreateDescriptorPool()
{
...
void VulkanRenderer::myCreateDescriptorSet()
{
...
void VulkanRenderer::myCreateCommandBuffers()
{
...
void VulkanRenderer::myCleanupSwapChain()
{
...
bool VulkanRenderer::myIsSuitableDevice(VkPhysicalDevice device)
{
...
void VulkanRenderer::myCreateSemaphores()
{
...
void VulkanRenderer::myCreateInstance()
{
...
void VulkanRenderer::myCreateLogicalDevice()
{
...
VkShaderModule VulkanRenderer::myCreateShaderModule(const std::vector<char>& byteCode)
{
